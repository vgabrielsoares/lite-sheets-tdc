...
export interface HPDetailSidebarProps {
  ...
  /** Breakdown de PV por arquétipo */
  archetypeBreakdown?: ArchetypeResourceBreakdown[];
  /** PV base inicial (ex: 15 para nível 1) */
  baseHP?: number;
}
...
export function HPDetailSidebar({
  ...
  const modifiers = hp.maxModifiers ?? [];
  ...
  const calculatedMax = baseHP + totalFromArchetypes + totalFromModifiers;

  // Atualizar o max quando os valores mudam
  React.useEffect(() => {
    if (hp.max !== calculatedMax) {
      onChange({ ...hp, max: calculatedMax });
    }
  }, [calculatedMax, hp, onChange]);
  ...
  return (
    <Sidebar open={open} onClose={onClose} title="Pontos de Vida">
      <Stack spacing={3}>
        {/* PV Atual e Temporário */}
        <Paper
          variant="outlined"
          ...
          >
            <Stack spacing={1.5}>
              ...
              <Stack
                ...
                alignItems="center"
              >
                <Typography variant="body2">PV Base</Typography>
                <Chip
                  label={baseHP}
                  size="small"
                  color="default"
                    ...
                    >
                      <Typography variant="body2" color="text.secondary">
                        ...
                        <Typography component="span" variant="caption">
                          (nv.{item.level} × [{item.basePerLevel}+CON])
                        </Typography>
                      </Typography>
                      <Chip
                        label={`+${item.total}`}
        ...
        <Typography
          ...
          sx={{ textAlign: 'center' }}
        >
          PV Máximo = {baseHP} (base) + {totalFromArchetypes} (arquétipos)
